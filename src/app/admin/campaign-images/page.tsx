...
import { Trash2, Plus, Image as ImageIcon, Loader2, Sparkles, Filter } from "lucide-react";
...
const BACKEND_API_URL = process.env.NEXT_PUBLIC_API_URL || "https://backend-api.richyreach.com/api";
...
export default function AdminCampaignImagesPage() {
  ...
  const { data: campaignImages = [], isLoading, error } = useQuery<CampaignImage[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        throw new Error("Failed to fetch campaign images");
      }
  ...
  // Delete campaign image mutation
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`${BACKEND_API_URL}/admin/campaign-images/${id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
        credentials: "include",
      });
  ...
  const categories = ["All", ...Array.from(new Set(campaignImages.map((img) => img.category)))];
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-10 px-4 md:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Manage category-wise campaign banners and preset templates used by brands.
          </p>
        </div>
        ...
        >
          {showAddForm ? "Cancel" : <><Plus className="w-4 h-4" /> Add Template</>}
        </button>
      </div>
      ...
      {showAddForm && (
        <div className="bg-white/80 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 shadow-xl max-w-xl animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">Upload New Template Image</h3>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800 rounded-3xl">
          <p className="text-slate-500 text-sm">Failed to load templates. Please try again later.</p>
        </div>
        ...
        <div className="text-center py-16 bg-white/40 dark:bg-slate-900/20 border border-slate-200/80 dark:border-slate-800/50 rounded-3xl flex flex-col items-center justify-center space-y-3">
          ...
          <div className="space-y-1">
            <h4 className="font-bold text-slate-900 dark:text-white text-base">No campaign templates</h4>
            <p className="text-xs text-slate-500">Upload preset banner designs to get started.</p>
          </div>
        </div>
      ) : (
        /* Image Grid */
          ...
          {filteredImages.map((img) => (
            ...
            >
              ...
              <div className="p-4 flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    if (window.confirm("Are you sure you want to delete this template image?")) {
